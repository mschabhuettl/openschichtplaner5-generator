...
function renderDemands(){
 ...
 const body=table(view.content,['Dienst und Zeit','Mindestens Personen','Höchstens Personen','Selber Posten','Details']);view.items.forEach(d=>{const tr=el('tr',undefined,body);el('td',demandLabel(d),tr);const min=field(el('td',undefined,tr),'Min',d.minimum,v=>d.minimum=v,'number');min.min='0';min.step='1';tableInputNavigation(min,'minimum');const maxInput=field(el('td',undefined,tr),'Max (leer = unbegrenzt)',d.maximum,v=>d.maximum=v,'number');maxInput.min='0';maxInput.step='1';maxInput.placeholder='Unbegrenzt';tableInputNavigation(maxInput,'maximum');const gruppe=field(el('td',undefined,tr),'Kennung (leer = eigener Posten)',d.alternative_group??'',v=>d.alternative_group=v.trim()||null);gruppe.placeholder='z. B. NCT-Nacht';gruppe.title='Bedarfe mit derselben Kennung decken denselben Posten. Der Posten gilt als besetzt, sobald eine der Varianten besetzt ist; eine zweite bleibt erlaubt.';tableInputNavigation(gruppe,'alternative');const details=el('details',undefined,el('td',undefined,tr));el('summary','Technische Details',details);el('p',d.id,details);el('p',d.source,details);el('p',workplaceName(dataIndex().positions.get(d.position_id)?.workplace_id??''),details);});
}
...
function renderRules(){
 ...
 renderProfiles();renderContext();renderUnresolved();$('weights').replaceChildren();el('p','Weniger Wechsel zwischen Arbeit und freien Kalendertagen fördern Dienstblöcke und zusammenhängende Freizeit. Beide Zeitraumränder mit bekannten Diensten werden berücksichtigt. Weiches Ziel, keine maximale Blocklänge; Freigaben, Bedarf, Sollstunden und harte Grenzen bleiben unverändert. Auch OPTIMAL bezieht sich auf die gewichtete Gesamtwertung, nicht auf eine garantiert längste Freizeit; FEASIBLE ist nicht optimalitätsbewiesen. 0 schaltet dieses Ziel aus.',$('weights'));el('p','Blockziele und gleichmäßige Verteilung ziehen gegeneinander, wenn die Dienste dünn gesät sind: Wo eine Person im Monat nur ein bis zwei Dienste hat, ist jeder Dienst zwangsläufig ein einzelner Arbeitstag. „Einzelne Arbeitstage" und „Arbeits-/Freizeitwechsel" drängen die Dienste dann auf wenige Personen zusammen. Für Bereitschafts- und Rufdienstpläne diese beiden Gewichte niedrig halten und stattdessen „Gleichmäßige Dienstanzahl" gewichten; für durchgehende Schichtpläne umgekehrt.',$('weights')).className='helper-text';button($('weights'),'Blockplanung aktivieren · Gewicht 400',()=>{snapshot.objectives.workday_transitions=400;invalidateResult();renderRules();notice('Weiches Blockziel aktiviert. Harte Regeln und Freigaben bleiben unverändert. Projekt speichern.');});Object.entries(snapshot.objectives).forEach(([k,v])=>field($('weights'),({workday_transitions:'Arbeits-/Freizeitwechsel · weiches Blockziel',isolated_days:'Einzelne Arbeitstage · weiches Blockziel',split_weekends:'Geteilte Wochenenden',block_shape:'Blocklängenprofil',hours:'Stunden',hours_fairness:'Gleichmäßige Sollerfüllung',duty_fairness:'Gleichmäßige Dienstanzahl',nights:'Nächte',weekends:'Wochenenden',holidays:'Feiertage',wishes:'Wünsche',changes:'Änderungen'})[k],v,n=>snapshot.objectives[k]=n,'number'));
}
...
const diagnosticActions={
 ...
 unresolved:'Unter Regeln und Bedarf die offenen Importangaben durchgehen und bestätigen.',
 availability:'Unter Team die Zeitfenster der betroffenen Person prüfen.',
...
function renderCalendar(){
 ...
 if(!byPosition)for(const work of snapshot.boundary_work??[]){
  ...
  // Work the source states without times occupies its day and says so.
  if(work.day){if(work.day>=start&&work.day<=last)touched.set(work.day,['ohne Zeitangabe']);}
  else for(const segment of work.segments){const first=localDay(segment.start),end=localDay(new Date(new Date(segment.end).getTime()-1));
