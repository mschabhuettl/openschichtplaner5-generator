...
**Version 0.9.27** erhöht den Kontrast der Hilfs- und Erklärungstexte für bessere Lesbarkeit. Die Anwendung macht importierte Vergleichsdienste nach Person, Dienst, Datum und Kennung durchsuchbar; Suche und Zuordnungsfilter lassen sich kombinieren. Die Anwendung lässt den gespeicherten Projekt- und Prüfstand beim Auswählen noch nicht übernommener Aktionen unverändert. Sie ergänzt die Tag-/Nacht-Automatik um eine prüfbare Vorschau mit Anzahl, Nachtminuten und nicht zuordenbaren Vorkommen. Erst die ausdrückliche Übernahme verändert offene Dienstarten. Die Anwendung zeigt bei offenen Angaben zu bestehenden Diensten Personnamen und direkte Prüfwege zu Person und Datum. Originalhinweise und Kennungssuche bleiben erhalten. Die Anwendung erhält vorhandene Einteilungen bei erfolglosen Berechnungen und unterscheidet technische Jobfertigstellung sichtbar vom geprüften Plan. Die automatische Vorprüfung filtert Vorprüfhinweise nach Kategorien und führt direkt zu Regelprofilen, Randkontext oder offenen Angaben. Gesamtzahl und Prüfstatus bleiben unabhängig vom Filter sichtbar. Die Folgezeitraum-Hilfe bereitet den Folgezeitraum mit passenden Kalenderdaten und Historienvorschlag vor, ohne einen Import oder eine Freigabe auszulösen. Die Vergleichsansicht führt von jedem importierten Vergleichsdienst direkt zur betroffenen Person, zum noch vorhandenen zugeordneten Bedarf oder zur Bedarfssuche am Datum. Navigation ändert weder Freigaben noch Regeln. Die gebündelten Verbesserungen machen große Listen offener Importangaben durch Suche und Zehnerseiten handhabbar; jede fachliche Klärung bleibt ausdrücklich einzeln. Übersicht und Berechnungsansicht zeigen denselben aktuellen Eingabeprüfstand. Importierte Vergleichsdienste bleiben mit Ursachenfilter und direkten Einrichtungsschritten nachvollziehbar. Normale Vergleichsdienste können ausdrücklich aus Ist oder Soll importiert werden. Historische Planbasis, Abwesenheiten, Sonderdienste und Randkontext bleiben getrennt. Die Zuordnungsübersicht führt zu den nötigen Einrichtungsschritten. Automatische Vorprüfung, personenbezogene Prüfhilfen und Sollstundenherkunft bleiben enthalten. Persönliche Dienstfreigaben bleiben der SP5-Standard; zusätzliche Qualifikationsanforderungen sind optional. [Änderungen und Aktualisierung](docs/release-0.9.27.md).
...
```sh
python -m pip install './openschichtplaner5_generator-0.9.27-py3-none-any.whl[web,sp5]'
sp5-generator serve --host 127.0.0.1 --port 8080 --state-dir ./generator-state
```
